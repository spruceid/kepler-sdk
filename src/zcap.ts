...
export const getHeaderAndDelId = <D>(
  delegation?: Delegation<D> | SiweMessage
) =>
  delegation instanceof SiweMessage
    ? {
        h: {
          "x-siwe-delegation": base64url.stringify(
            new TextEncoder().encode(
              JSON.stringify([delegation.toMessage(), delegation.signature])
            )
          ),
        },
    ...
    ? {
        h: {
          "x-kepler-delegation": base64url.stringify(
...
export const zcapAuthenticator = async <C extends Capabilities, D>(
  client: C,
  delegation?: Delegation<D> | SiweMessage
  ...
  return {
    content: async (
      ...
      const invstr = base64url.stringify(
        new TextEncoder().encode(JSON.stringify(inv))
      );
      return {
        [invHeaderStr]: invstr,
        ...h,
...
export const sessionProps = (
  ...
  invoker: string,
  capabilityAction: string[] = ["list", "get"],
  expiration: Date
) => ({
  parentCapability,
  invoker,
...
export const keplerContext = [
  W3ID_SECURITY_V2,
  { capabilityAction: { "@id": "sec:capabilityAction", "@type": "@json" } },
];
